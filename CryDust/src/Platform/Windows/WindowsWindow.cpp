...
namespace CryDust {
	...
	{
		CORE_DEBUG_ERROR("GLFW ERROR ({0}) : {1}" ,error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		CORE_DEBUG_INFO("Creating Windows {0} {1} {2}", props.Title, props.Width, props.Height);
		...
		{
			WindowData& data = *(WindowData*)glfwGetWindowUserPointer(window); //void*转换指针(c的方式)，这里传的是之前设置的m_Data
