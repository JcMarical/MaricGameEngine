...
namespace CryDust {
	...
	{
		std::string filepath = FileDialogs::OpenFile("CryDust Scene (*.crydust)\0*.crydust\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	{
		CORE_DEBUG_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
